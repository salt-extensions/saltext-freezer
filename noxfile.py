...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/freezer/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/freezer/*")
            ...
            #    "--include=src/saltext/freezer/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/freezer/config/schemas",
    )
